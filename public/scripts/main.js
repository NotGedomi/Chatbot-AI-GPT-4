  ...
  if (message) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        removeTypingMessage();
        addMessageToChat("bot", "Lo siento, ha ocurrido un error. Por favor, inténtalo de nuevo.");
      }
    } catch (error) {
...
function handleResponse(response) {
  if (response && typeof response === 'string') {
    ...
    const responseText = formattedResponse.text || formattedResponse; // Manejar tanto objetos como strings
    const role = "bot";
    ...
    addMessageToChat("bot", "Lo siento, no pude procesar la respuesta. Por favor, inténtalo de nuevo.");
  }
}
...
function formatResponse(response) {
  if (typeof response === "string") {
    ...
    return { text: formatLinks(formattedResponse) };
